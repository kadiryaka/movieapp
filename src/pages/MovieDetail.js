...
export default class MovieDetail extends Component {
	...
	componentDidMount = async() => {
		...
		let movie = getParam('movie');
		this.getAllMovieData(movie);
	};

	getAllMovieData = (movie) => {
		const promises = [
			fetchMovieDetailById({movieId:movie.id}),
			fetchMovieCreditsById({movieId:movie.id})
		];
	...
	renderMovieDetail() {
		let ratingTextList = this.state.movie.vote_average.toString().split(".");
		...
		return (
			...
			<View style={styles.container}>
					...
					<View>
						<ImageBackground source={{uri: this.state.backgroundImageUrl}} style={{width: '100%', height:'100%' }}>
							...
							<View style={styles.titleView}>
								<Text style={styles.title}> {this.state.movie.title.toUpperCase()} </Text>
							</View>
						</ImageBackground>
					</View>
				</View>
					...
					<View style={styles.genres}>
						<Text style={styles.genreText}>
						{
							this.state.movie.genres.map( (genre, i) => {
								return (this.state.movie.genres.length-1 === i) ? genre.name : genre.name + ", "
							})
						}
						</Text>
				...
				<View style={styles.descriptionView}>
					<Text style={styles.description} numberOfLines={5}>{this.state.movie.overview} </Text>
				</View>
